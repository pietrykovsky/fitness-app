...
DEV_RUN = "docker compose -f docker-compose.yml -f docker-compose.override.yml up --build"
...
def parse_args() -> dict[str, str]:
    parser = argparse.ArgumentParser(description="Set environment variables based on .env.sample")
    sample_args = {key: value for key, value in get_args()}
    for key, value in sample_args.items():
        parser.add_argument(f"--{key}", type=str, required=False, help=f"{value}", metavar="value")
    args = parser.parse_args()
    args_dict = {key: getattr(args, key) for key in sample_args.keys() if getattr(args, key) is not None}
    if len(args_dict) != len(sample_args.keys()) and len(args_dict) != 0:
        ...
        raise ValueError(f"Missing arguments: {', '.join(missing_args)}. Provide all arguments or none.")
    return args_dict
